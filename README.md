...
Analyze a healthcare encounter dataset to identify billing, utilization, admission, and test-result patterns. The project includes data cleaning, exploratory analysis, supervised modeling, Power BI dashboarding, and business interpretation.

## Dataset

Source: [Kaggle Healthcare Dataset by prasad22](https://www.kaggle.com/datasets/prasad22/healthcare-dataset)

This project uses the dataset for educational portfolio purposes. The dataset appears synthetic or anonymized, and this project does not treat it as real clinical evidence.
...
- Wrote SQL queries for validation, EDA, and SQL-only baseline modeling.
- Used Python for additional exploratory analysis and supervised modeling.
...
- Interpreted the results conservatively to avoid overstating predictive value.
...
The models did not find strong predictive signal in the available fields. This is an important analytical result: the dataset supports data cleaning, EDA, KPI reporting, and dashboarding, but it should not be presented as clinically predictive.

## Modeling Results

Two supervised modeling tasks were tested:

| Target | Task Type | Result | Interpretation |
|---|---|---:|---|
| Billing amount | Regression | R² ≈ 0.0000 | Available fields did not improve meaningfully over a simple baseline |
| Test results | Classification | Macro F1 = 0.3153 | Weak performance for a three-class classification problem |

The appropriate conclusion is that the dataset is better suited for **descriptive analytics** than prediction. Better predictive features would likely require clinical severity indicators, procedure codes, diagnosis codes, provider details, claim status, treatment duration, or payer contract information.
